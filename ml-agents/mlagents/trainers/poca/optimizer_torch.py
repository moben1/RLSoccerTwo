...
class TorchPOCAOptimizer(TorchOptimizer):
    ...
    def __init__(self, policy: TorchPolicy, trainer_settings: TrainerSettings):
        ...
        # Init hyperparameters that decay over iterations
        ...
        # Init optimizer (Adam : upgrade of stochastic gradient descent)
    ...
    def get_trajectory_and_baseline_value_estimates(
        ...
        # (Create lists of observations from the buffer)
        current_obs = ObsUtil.from_buffer(batch, n_obs)
        groupmate_obs = GroupObsUtil.from_buffer(batch, n_obs)
        ...
        # (convert next groupmate observations to tensor list)
        next_groupmate_obs = [
        ...
        # (Get agent memories or initialize them)
        ...
        # (Merge the observations from agent and groupmates)
        all_obs = (
        ...
        with torch.no_grad():  # (disable gradient calculation)
            # (estimate value and baseline for the trajectory)
            if self.policy.use_recurrent:
                # (allow to use intermediate memories for the critic)
                (
                    value_estimates,
                    baseline_estimates,
                    all_next_value_mem,
